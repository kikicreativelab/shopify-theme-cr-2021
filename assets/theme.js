    ...
    productVariantCallback: function(variant) {
      ...
      if (variant) {
        ...
        if (variant.available) {
          //alert('DEBUG: ' + variant.inventory_quantity);
          ...
          //this.settings.selectors.$addToCartText.html("Backordered");
        ...
        // Update variant title, sku, bin
        if (variant.title.toLowerCase().indexOf('default title') < 0) {
        ...
        if (variant.barcode) {
          $('#variant-bin').text('BIN: ' + variant.barcode);
        }
